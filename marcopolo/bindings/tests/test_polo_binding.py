...
class TestValidation(unittest.TestCase):
	def setUp(self):
		...
		self.polo.polo_socket.recv.return_value = ("{\"OK\": \"dummy\"}", 1)
		self.polo.polo_socket.recvfrom.return_value = 1
	...
	def test_recvfrom_timeout(self):
		self.polo.polo_socket.recv = MagicMock(side_effect=socket.timeout)
	...
	def test_bad_response(self):
		self.polo.polo_socket.recv = MagicMock(return_value=(-1, '127.0.0.1'))
		self.assertRaisesRegexp(polo.PoloInternalException, 
						"Error during internal communication",
						self.polo.publish_service,
						'dummy'
	...
	def test_valid_json(self):
		self.polo.polo_socket.recv = MagicMock(return_value=("{\"OK\": \"dummy\"}", '127.0.0.1'))
		valid_groups = set()
		valid_groups.add('224.2.2.2')
		valid_groups.add('224.2.2.3')
		valid_groups.add('224.2.2.4')
	...
	def test_valid_response(self):
		self.polo.polo_socket.recv = MagicMock(return_value=("{\"OK\": \"dummy:dummy\"}", '127.0.0.1'))
