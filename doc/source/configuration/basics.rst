...
- Different grids on the same network: MarcoPolo is bound to the multicast group 224.0.0.112 by default. In the event that this group is already in use by other MarcoPolo grid or any other application some problems may arise. Particularly on the first case, since MarcoPolo simply ignores any bad-formatted message (and the odds of a JSON-based mulicast application running on the same address are honestly quite small). The parameter `MULTICAST_ADDRS` in the file /etc/marcopolo/marcopolo.conf can be set to a list of compatible IPv4 multicast addresses.
