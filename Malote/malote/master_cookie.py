...
class DeployHandler(RequestHandler):
    def post(self):
        ...
        # Now that we know the request is fine, fire the updates
...
class LoginHandler(RequestHandler):
    ...
    def post(self):
        # TODO cambiar la pass
        if not compare_digest(self.get_argument("pwd"), "hola"):
            self.write("ERROR")
            return
        self.set_secure_cookie("user", self.get_argument("pwd"), expires_days=None)
        self.redirect("/")

class HomeView(StaticFileHandler):
    def initialize(self):
        super().initialize(path=static_path, default_filename='index.html')
    # Miau comprobar usuario
